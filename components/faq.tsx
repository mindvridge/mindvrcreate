  ...
  return (
    <section id="faq" className="border-t border-ink-line bg-ink-soft/40">
      <div className="mx-auto max-w-4xl px-5 py-24 sm:py-32">
        <Reveal>
          <p className="font-mono text-xs tracking-[0.25em] text-paper-faint">FAQ</p>
        ...
        <div className="mt-12 divide-y divide-ink-line border-y border-ink-line">
          {faqs.map((f) => (
            <details key={f.q} className="group py-2">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 py-4 text-base font-semibold marker:hidden [&::-webkit-details-marker]:hidden">
                {f.q}
                <span className="faq-chevron shrink-0 font-mono text-lg text-paper-faint transition-transform">
                  +
                </span>
              </summary>
              <p className="pb-5 leading-relaxed text-paper-dim">{f.a}</p>
            </details>
          ))}
        </div>
      </div>
